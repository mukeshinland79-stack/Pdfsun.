    ...
    <section className="py-12 bg-slate-50 dark:bg-slate-800/40 border-y border-slate-200 dark:border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-6">
        <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 text-xs font-bold">
          ...
          <span>{t("formats.badge", "Universal Document Converter")}</span>
        </div>

        <h2 className="text-2xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tight">
          {t("formats.title", "Supported File Formats on PDFSun")}
        </h2>
        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 max-w-xl mx-auto">
          {t("formats.subtitle", "Convert, process, and optimize documents across all major office formats, vector images, and eBook standards.")}
        </p>
          ...
          {SUPPORTED_FORMATS.map((fmt) => (
            ...
            >
              <span className="px-1.5 py-0.5 rounded text-[9px] font-extrabold uppercase bg-amber-500/10 text-amber-600 dark:text-amber-400">
                {t(`formats.badge_${fmt.ext}`, fmt.badge)}
              </span>
              ...
              <p className="text-[11px] text-slate-600 dark:text-slate-300 font-medium line-clamp-1">
                {t(`formats.desc_${fmt.ext}`, fmt.desc)}
              </p>
            </div>
          ))}
        </div>
