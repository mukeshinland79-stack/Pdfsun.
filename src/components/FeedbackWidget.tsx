...
export const FeedbackWidget: React.FC<FeedbackWidgetProps> = ({ toolId, toolName }) => {
  ...
  return (
    <div id="feedback-widget" className="mt-10 w-full max-w-5xl mx-auto space-y-6 animate-in fade-in duration-300 text-left">
      ...
      <div className="p-6 bg-[var(--bg-surface,#111114)] border border-[var(--border-color,rgba(255,255,255,0.1))] rounded-2xl shadow-sm space-y-6">
        
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 pb-6 border-b border-[var(--border-color,rgba(255,255,255,0.1))]">
          ...
          <div className="flex items-center space-x-5">
            <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-center min-w-[95px]">
              ...
              <div className="text-[10px] text-[var(--text-muted,#64748b)] font-medium">
                {stats.totalReviews} user ratings
              </div>
            </div>

            <div>
              <h2 className="text-base font-black text-[var(--text-primary,#f8fafc)] flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-3 mt-2 text-[11px] text-emerald-400 font-bold">
        ...
        {showReviewForm && (
          <form
            ...
            className="p-5 bg-[var(--bg-elevated,#16161a)] border border-[var(--border-color,rgba(255,255,255,0.1))] rounded-xl space-y-4 animate-in fade-in"
          >
            <div className="flex items-center justify-between pb-2 border-b border-[var(--border-color,rgba(255,255,255,0.1))]">
              <h3 className="text-xs font-black uppercase tracking-wider text-[var(--text-primary,#f8fafc)]">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-[var(--text-secondary,#94a3b8)] block">Select Rating</label>
              <div className="flex items-center space-x-1">
                ...
                <span className="ml-2 text-xs font-bold text-[var(--text-primary,#f8fafc)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-bold text-[var(--text-secondary,#94a3b8)] block mb-1">
                  Name <span className="text-[var(--text-muted,#64748b)] font-normal">(Optional)</span>
                </label>
                <input
                  type="text"
                  ...
                  maxLength={50}
                  className="w-full px-3 py-2 bg-[var(--input-bg,#111114)] border border-[var(--input-border,rgba(255,255,255,0.15))] rounded-lg text-xs text-[var(--text-primary,#f8fafc)] placeholder-[var(--text-muted,#64748b)] focus:ring-2 focus:ring-orange-500 focus:outline-none"
                />
              </div>

              <div>
                <label className="text-xs font-bold text-[var(--text-secondary,#94a3b8)] block mb-1">
                  Email <span className="text-[var(--text-muted,#64748b)] font-normal">(Private)</span>
                </label>
                <input
                  type="email"
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="text-xs font-bold text-[var(--text-secondary,#94a3b8)]">
                  Your Feedback *
                </label>
                <span className="text-[10px] text-[var(--text-muted,#64748b)]">{commentText.length} / 1000</span>
              </div>
              <textarea
                rows={3}
                ...
                maxLength={1000}
                required
                className="w-full px-3 py-2 bg-[var(--input-bg,#111114)] border border-[var(--input-border,rgba(255,255,255,0.15))] rounded-lg text-xs text-[var(--text-primary,#f8fafc)] placeholder-[var(--text-muted,#64748b)] focus:ring-2 focus:ring-orange-500 focus:outline-none resize-none"
              />
            </div>

            {/* CAPTCHA checkbox */}
            <div className="flex items-center justify-between p-3 bg-[var(--input-bg,#111114)] border border-[var(--border-color,rgba(255,255,255,0.1))] rounded-lg">
              <label className="flex items-center space-x-2.5 cursor-pointer">
                <input
                  type="checkbox"
                  checked={captchaChecked}
                  onChange={(e) => setCaptchaChecked(e.target.checked)}
                  className="w-4 h-4 text-orange-600 rounded border-[var(--border-color,rgba(255,255,255,0.2))] focus:ring-orange-500 bg-[var(--bg-elevated,#16161a)]"
                />
                <span className="text-xs font-semibold text-[var(--text-primary,#f8fafc)]">
                  I am human (Security CAPTCHA)
                </span>
              </label>
              <div className="flex items-center space-x-1 text-[10px] text-[var(--text-muted,#64748b)]">
            ...
            <div className="flex justify-end space-x-3 pt-2">
              <button
                ...
                className="px-4 py-2 text-xs font-bold text-[var(--text-secondary,#94a3b8)] hover:text-[var(--text-primary,#f8fafc)]"
              >
                Cancel
              </button>
              <button
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
          <div className="flex items-center space-x-2">
            <span className="text-xs font-bold text-[var(--text-secondary,#94a3b8)] flex items-center space-x-1">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-all ${
                selectedStarFilter === null
                  ...
                  : "bg-[var(--bg-elevated,#16161a)] text-[var(--text-secondary,#94a3b8)] hover:bg-[var(--bg-elevated-hover,#1f1f26)] hover:text-[var(--text-primary,#f8fafc)] border border-[var(--border-color,rgba(255,255,255,0.1))]"
              }`}
            >
              All
            </button>
          ...
          <div className="flex items-center space-x-2">
            <span className="text-xs text-[var(--text-muted,#64748b)]">Sort:</span>
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as "helpful" | "recent")}
              className="bg-[var(--bg-elevated,#16161a)] border border-[var(--border-color,rgba(255,255,255,0.1))] text-xs text-[var(--text-primary,#f8fafc)] rounded-lg px-2.5 py-1 font-semibold focus:outline-none cursor-pointer"
            >
        ...
        {/* 5. Reviews List */}
        {loadingComments ? (
          <div className="py-10 text-center text-[var(--text-muted,#64748b)] text-xs flex items-center justify-center space-x-2">
            <RefreshCw className="w-4 h-4 animate-spin text-orange-500" />
            <span>Loading reviews asynchronously...</span>
          </div>
        ) : filteredComments.length === 0 ? (
          <div className="py-10 text-center text-[var(--text-muted,#64748b)] text-xs space-y-2 border border-dashed border-[var(--border-color,rgba(255,255,255,0.1))] rounded-xl">
            <MessageSquare className="w-8 h-8 text-[var(--text-muted,#64748b)] mx-auto" />
            <p className="font-semibold text-[var(--text-secondary,#94a3b8)]">No reviews found for this filter.</p>
          ...
          <div className="space-y-4">
            {filteredComments.map((cmt) => (
              <div
                key={cmt.id}
                className="p-4 bg-[var(--bg-elevated,#16161a)] border border-[var(--border-color,rgba(255,255,255,0.1))] rounded-xl space-y-2.5"
              >
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        {cmt.verifiedUser !== false && (
                          <span className="inline-flex items-center space-x-1 px-1.5 py-0.2 rounded bg-emerald-500/10 text-emerald-400 text-[9px] font-black uppercase tracking-wider">
                            <ShieldCheck className="w-3 h-3 text-emerald-500" />
                            <span>Verified</span>
                          </span>
                        )}
                      </div>
                      <div className="text-[10px] text-[var(--text-muted,#64748b)]">
                        {new Date(cmt.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}
                      </div>
                    </div>
                  </div>

                  <div className="flex items-center space-x-0.5 text-amber-400 bg-[var(--bg-surface,#111114)] px-2 py-1 rounded-lg border border-[var(--border-color,rgba(255,255,255,0.1))]">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Star
                        key={s}
                        className={`w-3 h-3 ${s <= Math.round(cmt.rating) ? "fill-amber-400 text-amber-400" : "text-[var(--text-muted,#64748b)]"}`}
                      />
                    ))}
                  </div>
                </div>

                <p className="text-xs text-[var(--text-secondary,#94a3b8)] leading-relaxed font-normal whitespace-pre-wrap">
                  {cmt.comment}
                </p>

                <div className="flex items-center justify-between pt-1 border-t border-[var(--border-color,rgba(255,255,255,0.06))]">
                  <span className="text-[10px] text-[var(--text-muted,#64748b)]">Helpful review?</span>
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md text-[11px] font-semibold transition-all flex items-center space-x-1.5 ${
                      upvotedCommentIds.has(cmt.id)
                        ...
                        : "bg-[var(--bg-surface,#111114)] text-[var(--text-secondary,#94a3b8)] hover:text-orange-400 border border-[var(--border-color,rgba(255,255,255,0.1))]"
                    }`}
                  >
