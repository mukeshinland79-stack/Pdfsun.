...
import { GraduationCap, Landmark, ExternalLink, Award, CheckCircle2, BookOpen, TrendingUp, Sparkles } from "lucide-react";
  ...
  return (
    <section className={`my-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full ${className}`}>
      ...
      <div className="educational-ad-grid grid grid-cols-1 md:grid-cols-2 gap-6">
        {ads.map((ad) => {
          const readinessVal = ad.readinessPercentage ?? 95;
          const readinessTitle = ad.readinessLabel ?? "Academic & Career Readiness";

          return (
            <div
              key={ad.id}
              className="group relative rounded-2xl bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700 shadow-lg hover:shadow-2xl transition-all duration-300 flex flex-col justify-between overflow-hidden"
            >
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  {/* Dynamic Learning / Career Readiness Progress Bar */}
                  <div className="mb-5 p-3 rounded-xl bg-slate-50/80 dark:bg-slate-900/60 border border-slate-200/70 dark:border-slate-700/50">
                    <div className="flex items-center justify-between text-[11px] font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                      <span className="flex items-center gap-1">
                        ...
                        <span>{readinessTitle}</span>
                      </span>
                      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[10px] font-extrabold border border-amber-500/20">
                    ...
                    {/* Progress Track */}
                    <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-2 overflow-hidden p-0.5">
                      <div
                        className={`h-full rounded-full bg-gradient-to-r ${ad.accentGradient} transition-all duration-1000 ease-out shadow-sm`}
